  ...
  if (loading) {
    return (
      <main className="stock-details-page">
        <div className="loading-state" aria-live="polite">Loading {symbol}…</div>
      </main>
    );
  }
  ...
  return (
    <main className="stock-details-page">
      ...
      <div className="details-grid">
        ...
        <div className="details-main">
          ...
          <section className="stock-header" aria-label="Stock summary">
            <div className="stock-header-left">
              ...
              {overview?.Name && <p className="stock-full-name">{overview.Name}</p>}
              {overview?.Sector && (
                <span className="sector-badge">{overview.Sector}</span>
              )}
            </div>
            <div className="stock-header-right">
              <span className="detail-price">{formatCurrency(quote?.price)}</span>
                ...
                <button
                  ...
                  onClick={() =>
                    inWatchlist
                      ...
                      : addToWatchlist({ symbol, name: overview?.Name ?? symbol })
                  }
                >
          ...
          <section className="key-stats" aria-label="Key statistics">
            {[
              ...
              { label: 'Mkt Cap', value: formatCompact(overview?.MarketCapitalization) },
                ...
                <AreaChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                  <defs>
                    <linearGradient id="priceGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={priceClass === 'negative' ? '#F85149' : '#3FB950'} stopOpacity={0.3} />
                      <stop offset="95%" stopColor={priceClass === 'negative' ? '#F85149' : '#3FB950'} stopOpacity={0} />
                    </linearGradient>
                  </defs>
        ...
        {/* Right column: Chat */}
        <aside className="details-chat" aria-label="Stock discussion">
          <h2 className="section-title">Discuss {symbol}</h2>
      ...
      {/* Buy Modal */}
      {buyModalOpen && (
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-label="Add to portfolio">
          <div className="modal">
            <button className="modal-close" onClick={() => setBuyModalOpen(false)} aria-label="Close">✕</button>
            <h2 className="modal-title">Add {symbol} to Portfolio</h2>
            <p className="modal-price">Current price: <strong>{formatCurrency(quote?.price)}</strong></p>
            <form onSubmit={handleBuy} className="modal-form">
              <label htmlFor="shares-input" className="modal-label">Number of shares</label>
              <input
                id="shares-input"
                type="number"
              ...
              {buyShares && !isNaN(buyShares) && quote?.price && (
                <p className="modal-total">
                  Total: <strong>{formatCurrency(parseFloat(buyShares) * quote.price)}</strong>
                </p>
              )}
              <button type="submit" className="btn-primary btn-block">Confirm Purchase</button>
            </form>
          </div>
        </div>
      )}
