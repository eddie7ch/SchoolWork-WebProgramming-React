...
class Character {
    ...
    getStatus() {
        const healthBar = 'Γûê'.repeat(Math.floor((this.health / this.maxHealth) * 10));
        const emptyBar = 'Γûæ'.repeat(10 - Math.floor((this.health / this.maxHealth) * 10));
...
class Location {
    ...
    searchLocation() {
        ...
        if (Math.random() < 0.3 && !this.searched) {
            ...
            results.push(`≡ƒöì You discovered a hidden ${randomItem.name}!`);
            this.searched = true;
        }
...
class Game {
    ...
    pickUpItem(itemName) {
        ...
        if (item.type === 'treasure') {
            ...
            if (this.treasuresFound >= 2) {
                this.gameWon = true;
                this.gameOver = true;
                return result + '\n\n≡ƒÄë CONGRATULATIONS! You have collected enough treasures and won the game! ≡ƒÄë';
            }
        }
    ...
    attackCharacter(characterName) {
        ...
        if (character.isAlive) {
            ...
            if (!this.player.isAlive) {
                this.gameOver = true;
                result += '\n\n≡ƒÆÇ GAME OVER! Your adventure ends here... ≡ƒÆÇ';
            }
        } else {
    ...
    listVisitedLocations() {
        ...
        return 'Visited locations:\n' + visitedLocations.map(loc => 
            `ΓÇó ${loc.name} - ${loc.description.substring(0, 60)}...`
        ).join('\n');
    }

    getHelp() {
        return `
≡ƒùí∩╕Å SCRIPTORIA COMMANDS ≡ƒùí∩╕Å

Movement:
ΓÇó move [location] / go [location] - Travel to a connected location
ΓÇó look / examine - Look around current location

Items:
ΓÇó pick [item] / take [item] - Pick up an item
ΓÇó search - Search for hidden items
ΓÇó inventory / inv - View your inventory

Combat:
ΓÇó attack [character] / fight [character] - Attack a character
ΓÇó talk [character] / speak [character] - Talk to a character

Status:
ΓÇó status / health - View your current status
ΓÇó locations - List visited locations
ΓÇó help - Show this help message

≡ƒÄ» GOAL: Collect treasures and explore the mysterious land of Scriptoria!
        `;
    }
...
function gameLoop() {
    if (game.gameOver) {
        const endMessage = game.gameWon 
            ? '\n\n≡ƒÄë Congratulations! You have mastered Scriptoria! ≡ƒÄë'
            : '\n\n≡ƒÆÇ Game Over! Refresh the page to try again. ≡ƒÆÇ';
